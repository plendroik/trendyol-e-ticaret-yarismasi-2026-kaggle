# =============================================================================
# VAST.AI (RTX 3090/4090/A100) UZERINDE mDEBERTa-v3-large EGITIMI
#
# Bu script, microsoft/mdeberta-v3-large modelini Vast.ai kiralık GPU'sunda
# gradient collapse yasamadan float16 mixed precision ile egitir.
# CIKTI: mdeberta_large_test.npy + mdeberta_large_holdout.npy
# =============================================================================
...
MAXLEN = 160
TRB = 16          # Large model icin 24GB VRAM'de batch size 16 uygundur
INB = 128
EPOCHS = 2
LR = 7e-6         # DeBERTa-Large icin kararli ogrenme orani
HOLDOUT = 4
dev = "cuda"
...
def main():
    ...
    log(f"Egitim basliyor: {EPOCHS} epoch, BatchSize={TRB}, LR={LR} (float16 autocast)")
    model.train()
    ...
    log("Holdout skorlaniyor...")
    ho = score(model, hol, coll_te)
    np.save(os.path.join(HERE, "mdeberta_large_holdout.npy"), ho)
    
    from sklearn.metrics import f1_score
    y = hol.label.to_numpy()
    best = max((f1_score(y, (ho >= t).astype(int), average="macro"), t) for t in np.arange(0.3, 0.71, 0.05))
    log(f"HOLDOUT macroF1={best[0]:.5f} @ {best[1]:.2f}")
    
    log("Test skorlaniyor (3.36M)...")
    ts = score(model, te, coll_te)
    np.save(os.path.join(HERE, "mdeberta_large_test.npy"), ts)
    log("BITTI -> mdeberta_large_test.npy + mdeberta_large_holdout.npy")

@torch.no_grad()
def score(model, df, coll_fn):
    model.eval()
    dl = DataLoader(DS(df.q.tolist(), df.d.tolist()), batch_size=INB, collate_fn=coll_fn,
