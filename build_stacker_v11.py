# =============================================================================
# STACKER v11: 13-Model Ensemble (Old Models + New 72B Models)
#
# Bu script, önceki basarili CatBoost/LightGBM stacker yapisini alip:
#   1. Yeni egitilen 72b_domain, 72b_tybert ve 72b_xlmrL2 (saglikli olan) modellerini ekler.
#   2. Etiketleri su oncelik sirasiyla birlestirir:
#      test_judge_labels -> rescue_labels -> clean_labels_72b (Qwen3-8B) -> gpt4o_labels
#   3. LightGBM + CatBoost modellerini egitip, olasiliklari harmanlar.
#   4. Karar esigini OOF macro-F1'i maksimize edecek sekilde tarar.
# =============================================================================
...
# Yeni 72B modelleri
for name in ["72b_domain", "72b_tybert", "72b_xlmrL2"]:
    p = os.path.join(ART, f"ce_test_{name}.npy")
    if os.path.exists(p):
        S[name] = np.load(p).astype(np.float32)
        log(f"  + {name} (72b)")
...
log("Labels resolving (Hierarchy)...")
id2lab = {}
...
# 2. Qwen3-8B High-quality direct labels (Overrides old labels where available)
...
# 3. GPT-4o High-quality referee labels (Final word, overrides all)
for f in ["gpt4o_labels.csv", "disputed_gpt_4o.csv"]:
    p = os.path.join(DATA, f)
    if os.path.exists(p):
        d = pd.read_csv(p)
        for i_, l_ in zip(d["id"].astype(str), d["label"].astype(int)):
...
for k in range(n):
    ...
    if v is not None:
        y[k] = v
mask = y >= 0
log(f"Labeled rows: {mask.sum():,}  pos_rate={y[mask].mean():.4f}")

# Grouped folds by term_id
fold = np.array([zlib.crc32(str(t).encode()) % 5 for t in tids], dtype=np.int8)

log("Training LGBM + CatBoost Stacker...")
