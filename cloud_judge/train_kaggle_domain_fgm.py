# =============================================================================
# KAGGLE (T4 GPU) UZERINDE UCRETSIZ DOMAIN MODEL + FGM (ADVERSARIAL) EGITIMI
#
# Bu script, Trendyol domain modelini (TY-ecomm-embed-multilingual-base-v1.2.0)
# FGM (Fast Gradient Method) adversarial regularizasyonu ile egitir.
# FGM, modelin yazım hatalarına ve kelime sırası degisimlerine dayanıklılığını artırır.
# =============================================================================
...
def main():
    ...
    log(f"Model yukleniyor: {MODEL} (FGM: ON)")
    ...
    log(f"Egitim basliyor: {EPOCHS} epoch, BatchSize={BATCH_SIZE} (FGM=ON)")
    ...
    for ep in range(EPOCHS):
        ...
        for i, b in enumerate(dl):
            ...
            # Standart backward pass
            with torch.amp.autocast('cuda', dtype=torch.float16):
                out = model(input_ids=b["input_ids"], attention_mask=b["attention_mask"])
                loss = lossf(out.logits, b["labels"])
            scaler.scale(loss).backward()
            
            # Adversarial attack (FGM) pass
            fgm.attack()
            with torch.amp.autocast('cuda', dtype=torch.float16):
                out_adv = model(input_ids=b["input_ids"], attention_mask=b["attention_mask"])
                loss_adv = lossf(out_adv.logits, b["labels"])
    ...
    ho = score(hol)
    np.save("/kaggle/working/domain_fgm_holdout.npy", ho)
    
    from sklearn.metrics import f1_score
    y = hol.label.to_numpy()
    best = max((f1_score(y, (ho >= t).astype(int), average="macro"), t) for t in np.arange(0.3, 0.71, 0.05))
    log(f"Holdout FGM macro-F1: {best[0]:.5f} at threshold: {best[1]:.2f}")
    
    log("Test setini skorlama (3.36M)...")
    ts = score(te)
    np.save("/kaggle/working/domain_fgm_test.npy", ts)
