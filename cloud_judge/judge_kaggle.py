...
SYS = ("Sen Trendyol arama-alaka uzmanısın. Verilen (sorgu | ürün) çifti için ürünün "
       ...
       "ürün 0. Kararsız kalırsan 1 ver. SADECE tek karakter yaz: 0 ya da 1.")
...
# ---- 2) ETIKETLEME ----------------------------------------------------------
idsf = sorted(glob.glob(os.path.join(IDS_DIR, f"band_ids_part{PART}.csv*")))[0]
ids = pd.read_csv(idsf)["id"].astype(str).tolist()
print(f"part {PART}: {len(ids):,} cift")
OUT = f"/kaggle/working/labels_part{PART}.csv"
done = set()
